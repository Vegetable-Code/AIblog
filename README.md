...
基于 Vue 3 + FastAPI 的全栈个人博客，支持 Markdown 写作、PDF 导入、AI 搜索、作品集展示、日程管理、多语言国际化。
...
| 认证 | JWT Token |
| 存储 | 本地文件系统 / S3 兼容对象存储（Cloudflare R2 已配置） |
| 部署 | Docker + Railway |
...
- **AI 智能搜索** — 基于语义的全文搜索
- **图形验证码** — 防止机器人刷接口
- **多语言国际化** — 根据 IP 自动切换中/英文/日文
- **主题切换** — 管理后台支持暗色/亮色模式
- **标签管理** — 编辑文章时可直接创建新标签
- **作品集** — 展示个人项目，支持封面图、多图轮播、标签
- **日程管理** — 日历 + 本日日程，用户隔离，登录后使用
- **词云** — 首页标签云，动态缩放、可拖拽
- **响应式设计** — 适配桌面和移动端
...
├── backend/               # FastAPI 后端
│   ├── app/
│   │   ├── api/           # API 路由（文章、分类、标签、评论、认证、作品集、日程、PDF导入、AI搜索等）
│   │   ├── models/        # 数据库模型（文章、分类、标签、评论、用户、作品集、日程）
│   │   ├── schemas/       # Pydantic 数据模型
│   │   └── core/          # 配置、数据库、安全、依赖、存储层（S3/本地）
│   ├── requirements.txt
│   ├── init_user.py       # 初始化管理员脚本
│   └── migrate_to_s3.py   # 迁移本地文件到 S3 脚本
├── frontend/              # Vue 3 前台
│   └── src/
│       ├── views/         # 页面视图（首页、分类、文章详情、作品集、关于、标签文章）
│       ├── components/    # 组件（AI助手、评论、登录弹窗、Toast）
│       ├── i18n/          # 国际化资源（zh-CN, en-US, ja-JP）
│       └── stores/        # Pinia 状态管理
├── admin/                 # Vue 3 后台管理
│   └── src/
│       ├── views/         # 管理页面（仪表盘、文章、分类、标签、评论、用户、作品集、PDF导入、个人资料）
│       └── styles/        # 全局样式（暗色/亮色主题）
...
| GET | /api/v1/projects/public | 获取作品集列表 |
| GET | /api/v1/schedules | 获取日程（需登录） |
| POST | /api/v1/schedules | 添加日程 |
| GET | /api/v1/ai-search | AI 语义搜索 |

## 对象存储（R2）

项目已配置 Cloudflare R2 作为对象存储后端，用于保存文章图片和作品集图片，确保部署不会丢失文件。

R2 配置通过以下环境变量控制（配置后自动启用，不配置则回退到本地文件系统）：
...
项目已配置 Railway 部署，详见 Dockerfile 和 ailway.json。
